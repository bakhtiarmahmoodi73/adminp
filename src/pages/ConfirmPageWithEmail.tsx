  ...
  const loadFromLocalStorage = () => {
    ...
    if (savedData) {
      try {
        ...
        const isRecent =
          new Date().getTime() - parsedData.timestamp < 10 * 60 * 1000;
  ...
  const formik = useFormik<ConfirmFormData>({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (
          exchangeState.fromCurrency === "tether" &&
          exchangeState.toCurrency === "permoney"
        ) {
          navigate("/flow/send");
        } else if (
          exchangeState.fromCurrency === "permoney" &&
          exchangeState.toCurrency === "tether"
        ) {
    ...
    <ContainerConfirm sx={{ height: containerHeight }}>
      ...
      <BoxConfirmRoot sx={{ mt: "31px", gap: "12px" }}>
        ...
        <BoxDetail sx={{ gap: "13px", justifyContent: "normal" }}>
          ...
          <TypographyRule>
            Read The Terms Of Exchange. If You Accept Them, Check The
            Appropriate Boxes. Pay For The Application According To The
        ...
        >
          <CustomCheckbox
            disableRipple
            icon={<span className="icon-box" />}
            checkedIcon={<span className="checked-box" />}
            name="agreedToTerms"
          ...
          <TypographyRule>
            I Agree With The{" "}
            <span style={{ color: "#60A7F8", cursor: "pointer" }}>
              AML Policy
            </span>{" "}
            And{" "}
            <span style={{ color: "#60A7F8", cursor: "pointer" }}>
        ...
        <CustomButton
          ...
          sx={{
            ...
            cursor:
              formik.isSubmitting || !isFormValid() ? "not-allowed" : "pointer",
          }}
        >
